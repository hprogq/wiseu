...
import service from "../models/Service";
import { addServiceTask, removeServiceTask } from '../services/service/serviceScheduler';  // 引入定时任务取消方法
    ...
    restful(req, res, {
        get: async (req: Request, res: Response) => {
            try {
                ...
                res.json(createResponse(true, 'Available services fetched successfully', services));
            } catch (error: any) {
    ...
    restful(req, res, {
        get: async (req: Request, res: Response) => {
            try {
                ...
                res.json(createResponse(true, 'User services fetched successfully', services));
            } catch (error: any) {
                res.status(500).json(createResponse(false, error.message));
            }
        },
        ...
        post: async (req: Request, res: Response) => {
            try {
                ...
                if (!type || !identityId || !configuration || !isConfiguration(configuration)) {
                    return res.status(400).json(createResponse(false, 'Invalid request body'));
                }
                ...
                if (!mongoose.Types.ObjectId.isValid(identityId)) {
                    return res.status(400).json(createResponse(false, 'Invalid identity ID'));
                }
                const identity = await Identity.findOne({ _id: new mongoose.Types.ObjectId(identityId), user: new mongoose.Types.ObjectId(userId) });
                if (!identity) {
                    return res.status(404).json(createResponse(false, 'Identity not found'));
                }
                if (!serviceInstance.identityType.includes(identity.type)) {
                    return res.status(400).json(createResponse(false, 'Invalid identity type for this service'));
                }

                // 检查服务是否已存在
                const existingService = await Service.findOne({ user: userId, type, identityId: new mongoose.Types.ObjectId(identityId) });
                if (existingService) {
                    return res.status(400).json(createResponse(false, 'Service already exists'));
                }
                ...
                res.json(createResponse(true, 'Service added successfully', { id: newService.id }));
            } catch (error: any) {
                res.status(500).json(createResponse(false, error.message));
            }
        }
    ...
    restful(req, res, {
        delete: async (req: Request, res: Response) => {
            try {
                ...
                if (!service) {
                    return res.status(404).json(createResponse(false, 'Service not found'));
                }
    ...
    restful(req, res, {
        put: async (req: Request, res: Response) => {
            try {
                ...
                if (!serviceInstance.identityType.includes(identity.type)) {
                    return res.status(400).json(createResponse(false, 'Invalid service type for this service'));
                }
                ...
                res.json(createResponse(true, 'Service configuration updated successfully', service));
            } catch (error: any) {
                res.status(500).json(createResponse(false, error.message));
            }
        }
    ...
    restful(req, res, {
        get: async (req: Request, res: Response) => {
            try {
                ...
                const service = await Service.findOne({ _id: serviceId, user: userId }).populate('identity');
                if (!service) {
                    return res.status(404).json(createResponse(false, 'Service not found'));
                }
                ...
                const isValid = await serviceInstance.init(service.identityId, service.configuration);
                if (!isValid) {
                    return res.status(400).json(createResponse(false, 'Invalid service configuration'));
                }
        ...
        post: async (req: Request, res: Response) => {
            try {
                ...
                if (service.status === 'UP') {
                    return res.status(400).json(createResponse(false, 'Service is already enabled'));
                }
        ...
        post: async (req: Request, res: Response) => {
            try {
                ...
                if (service.status === 'DISABLED') {
                    return res.status(400).json(createResponse(false, 'Service is already disabled'));
                }
    ...
    restful(req, res, {
        get: async (req: Request, res: Response) => {
            try {
                ...
                res.json(createResponse(true, 'Service details fetched successfully', service));
            } catch (error: any) {
                res.status(500).json(createResponse(false, error.message));
            }
        }
        ...
        post: async (req: Request, res: Response) => {
            try {
                ...
                if (interval > 0) {
                    ...
                    res.json(createResponse(true, 'Service refreshed and next execution scheduled.'));
                } else {
                    res.json(createResponse(true, 'Service refreshed but no scheduling needed due to interval <= 0.'));
                }
            } catch (error: any) {
                res.status(500).json(createResponse(false, error.message));
            }
